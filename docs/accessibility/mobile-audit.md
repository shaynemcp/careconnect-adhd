...
| 3 | Medium | `SignInScreen.tsx` lines 12 and 56 | Uses React Native's deprecated `SafeAreaView`, which only insets on iOS. With Android edge-to-edge (default on current Expo) the "CareConnect" heading may sit under the status bar. | Import `SafeAreaView` from `react-native-safe-area-context`, as the rest of the app already does through its provider. |
