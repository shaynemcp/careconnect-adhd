...
| 2026-XX-XX | *(template — paste over this row once the TalkBack pass runs)* Mobile RN — sign-in order, undo timing, switch reachability, read-only field label, touch targets, dose-action labels | Manual | TalkBack (Android, `<device/emulator>`) | `<PASS/FAIL>` — `<n>/<n>` checks from `docs/accessibility/talkback-live-pass.md`, notes: `<anything that didn't match expected>` | Quinton |
